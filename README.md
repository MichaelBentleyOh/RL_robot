...
### 2. depth 정규화
큐브까지의 거리를 계산하기 위해 `arm_camera()`의 `depth_img`에서 depth 값을 추출하였으나, 해당 값이 정규화되지 않은 값임을 발견하였습니다. 이를 해결하기 위해 설정된 `far` 값을 활용하여 `depth = depth_value / 255.0 * far`로 정규화를 진행하였고, 문제를 해결하였습니다.
